...
public class Bcontroller {
	...
	public boolean delete(int bno) {
		boardDB.remove(bno);		// 인수로 전달받은 인덱스[게시물의 번호]의 게시물을 삭제
		return true;
...
/*
5. 글쓰기 처리(회원제 : 로그인 했다는 가정하에)
	인수 : title , content		반환 : true 성공 false 실패

6. 글출력 데이터 반환
	인수 : X [ 페이징처리 , 검색처리 ] ->나중에//	반환 : 모든 글이 담긴 ArrayList(board 여러개라서 list)	--> 5개월뒤에 혼자 쉽게 할 수 있어야 함

7. 글상세 데이터 반환
	인수 : 글번호					반환 : 글 하나 board하나
	* 조회수 증가
8. 글삭제 처리
	인수 : 글번호					반환 : true 성공 false 실패

9. 글수정 처리
	인수 : 글번호 , 새로운제목 ,새로운 내용	반환 : true 성공 false 실패
*/