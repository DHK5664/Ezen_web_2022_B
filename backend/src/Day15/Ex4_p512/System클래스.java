...
public class System클래스 {
	...
	public static void main(String[] args) {
		...
		// 1. 
		try { // try{} 에 에러가 발생할 것 같은 코드[*경험] 
		int value4 = Integer.parseInt("1xx");	// "1xx" --> 100 변환 불가
		}
		...
		while(true) {
			...
			if(keyCode != 13 && keyCode !=10 ) { // 엔터[13,10]가 아니고
				if(keyCode == 49 ) {// 숫자1의 코드가 49
					speed++;
				}else if(keyCode == 50) { // 숫자 2 코드는 50
					speed--;
				}else if(keyCode == 51) { // 숫자 3 코드 51
					//break;
					System.exit(0);	// 0:정상종료 , 1 or -1 : 비정상종료
				}
