...
public class Ex1_p529 {
	public static void main(String[] args) {
		...
		// Math 클래스를 컨트롤+좌클릭 해보면 얘는 static이라 new없이 사용 가능하다.
			// 1. ceil() : 올림	[ 소수 첫째자리 ]
			// 2. floor() : 버림	[ 소수 첫째자리 ]
		...
		int[] selectNumber = new int[6];
				// vs
		// int형 변수를 6개 저장할 수 있는 리스트	[ 가변길이 , 메소드제공O , 기본길이=10 그런데 add할때마다 1씩 증가 ]
		ArrayList<Integer> selectNumber2 = new ArrayList<>();
			// 포장 클래스 : 기본자료형 --> 클래스 [ *특정 API클래스 중 기본자료형 사용불가 ]
		...
		//
		int[] winningNunber = new int[6];
		random = new Random();
		System.out.println("당첨번호 : ");
		for(int i = 0 ; i<6 ; i++) {
			winningNunber[i] = random.nextInt(45)+1;
			System.out.print(winningNunber[i]+" ");
		}
		System.out.println();
		...
		Arrays.sort(winningNunber);
		System.out.println(Arrays.toString(winningNunber));
		boolean result = Arrays.equals(selectNumber, winningNunber);
