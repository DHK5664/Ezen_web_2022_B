...
public class Ex2_입력 {	// c s
	...
	public static void main(String[] args) { // m s
		...
		// 2.입력메소드 -> 변수 저장 [ 자료형 맞추기 ]
			...
			// 2. 문자열[띄어쓰기 가능] 입력받기 함수 [.nextLine()앞에 다른 next() 있으면 하나로 취급 -> 해결방안 : 앞에 .nextLine() 작성]
			...
			// 4. 첫글자만 입력받기 함수 .next() 문자열 중에서 charAt(0)
			char 문자 = scanner.next().charAt(0); System.out.println("문자 : " +문자);	
			// 5. 정수형 입력받기
			byte 바이트 = scanner.nextByte();	System.out.println("바이트 : " +바이트);			
			short 쇼츠 = scanner.nextShort();	 System.out.println("쇼츠 : " +쇼츠);	// 6. 정수형 입력받기
			int 인트 = scanner.nextInt();		System.out.println("인트 : " +인트);	// 7. int정수형으로 입력받기
			long 롱 = scanner.nextLong();	System.out.println("롱 : " +롱);		// 8. 정수형 입력받기
			// 9. 실수형 입력받기
			...
			// 문자열[String] 비교	[ 객체는 비교 연산자 불가능 -> 문자열 A.equals("문자열B")]
