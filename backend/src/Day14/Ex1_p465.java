	...
	public static void printLength(String data) {
		try {	// * 예외 발생할 것 같은 코드를 try {} 안에 넣기
			...
			// * 만약에 try { } 안에서 예외가 발생했을때 실행되는 catch { } 코드
				// - 만일 try{} 안에서 예외 발생하지 않으면 catch {} 실행 안됨.
		...
		// 1. 메인함수 선언 : 메인스레드가 포함된 함수 -> 스레드 : 코드를 읽어주는 흐름단위 [ 단일 스레드 ]
	public static void main(String[] args) {
		
		System.out.println("프로그램시작");
		printLength("ThisIsJava");
		printLength(null);			// NullPointerException : 데이터가 null이라서 length()불가
...
/*
	...
	문법
		try{
			1. 예외발생 할 것 같은 코드[ 경험,입출력(키보드도 자바기준으론 외부라서) ]
		}
		catch( 예외클래스 e ){
