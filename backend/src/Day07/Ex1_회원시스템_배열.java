	...
	public static void main(String[] args) {// m s	
		...
		while(true) {// 무한루프 [종료조건 : 없음]
			...
			if(ch==1) {// 1입력했을때
				...
				memberList[no] = null;
				// * 삭제 후 빈자리 채우기 [ 삭제된 인덱스 뒤로 한칸씩 당기기 ]
				for(int i = no ; i<memberList.length ; i++) {	// no를 넣은 이유는 삭제된 자리 앞쪽은 가만히 있어도 되고 뒤에만 앞으로 당기면 되니까
						if( i+1 == memberList.length) {// 마지막인덱스 이면
							memberList[i] = null; break;	// 마지막인덱스 null 넣고 나가기
						}
