	...
	-SQL 질의어
        ...
        2. DML
            ...
            select
                ...
                테이블의 조건 검색 : select * from 테이블명 where
            update
				모든 레코드에 대한 수정 : update 테이블명 set 필드명 = 새로운값 , 필드명2 = 새로운값
                특정 레코드에 대한 수정 : update 테이블명 set 필드명 = 새로운값 , 필드명2 = 새로운값 where 조건
            delete
...
create table member(
	mno int auto_increment primary key ,
			-- auto_increment : 자동번호 부여 [*pk 사용]	auto_increment는 pk와 세트 그래야 자동번호 부여가 됨
...
-- 7. 로그인하기 , 아이디와 비밀번호가 일치하면 [ 'asd' , 'zxc' ]
...
delete from member where mno = 2; -- mno 2번인 레코드 삭제
