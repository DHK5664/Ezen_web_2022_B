...
create table member(
	...
	mid char(8) primary key,
		-- char(길이) : 길이만큼 문자 저장 : [ 최대 길이만큼 글자 ]
        -- not null : 공백 저장 불가능 [ 만일 공백이면 저장 실패 ]
        -- primary key : (기본키) 식별키 [ 필드 내 중복 불가능 , 공백불가능 ]
...
-- ER 다이어그램 [ 테이블 간 관계 확인 ]
	-- 메뉴 -> database -> Reverse Engineer -> 중간에 로그인 하고 DB 선택 -> 다음
...
-- 4. select 필드명 from 테이블명 where 조건
    ...
    select * from member where mnumber = 4;			-- 멤버 필드가 4이면 레코드 검색(출력)
    ...
    select * from member where mheight >= 163 and mheight <=165;	-- 키 필드가 163~165
    select * from member where mheight between 163 and 165;			-- 키 필드가 163~165
    select * from member where maddr = '경기' or maddr = '전남' or maddr = '경남';	-- 주소가 경기 혹은 전남 혹은 경남 중 하나이면
    ...
    select * from member where mname like '우_';		-- '우' 로 시작하는 두 글자 찾기
    select * from member where mname like '__우';	-- '우' 로 끝나는 세글자 찾기
    select * from member where mname like '_우_';	-- 세글자 중 가운데 글자가 '우' 인 문자 찾기
    select	mnumber 원본필드 ,mnumber+1 더하기 , mnumber-1 빼기, mnumber*2 곱하기, mnumber/2 나누기, mnumber div 2 몫, mnumber mod 2 나머지
    from member;
    select mnumber * mheight 멤버키점수 from member;
    
-- 5. select * from 테이블명 order by 정렬기준필드
	-- select * from 테이블명 where 조건 order by 정렬기준 / order by는 항상 마지막
    -- 다중 정렬 : 앞 정렬 후 동일한 데이터가 있을경우 하위 정렬
		-- select * from 테이블명 order by 정렬기준필드1 , 정렬기준필드2
	select * from member order by mdebut asc;	-- 과거순 -오름차순	[ asc 생략가능 ]
    select * from member order by mdebut desc;	-- 최신순 -내림차순	[ 과거 날짜가 더 작음 / 최근날짜가 더 크다 1900-01-01을 1로 보고 하루마다 1을 더하기 때문]
...
-- 8. select * from 테이블명 group by 그룹기준필드
    ...
    -- 4. 
    select mid 회원아이디 , count(*) as 결제한수 from buy group by mid;
    
-- 9. select * from 테이블명 group by 그룹필드기준 having 그룹 내 조건
	-- * 회원 아이디[그룹] 별 로 총매출액 [ 가격*수량 ] 합계가 1000 이상 검색
    ...
    select mid 회원아이디 , sum(bprice * bamout)as 총매출액
    from buy
    where sum(bprice*bamout)>= 1000		-- 오류발생 : sum() 집계함수 이므로 그룹 후에 조건 사용 가능
    group by mid;
...
-- insert
	-- 1. 특정 필드에 값 삽입 insert into 테이블명(필드명1 , 필드명2) values(값1 , 값2);
    -- 2. 전체 필드에 값 삽입 insert into 테이블명 values(값1 , 값2);
    -- 3. 다중 레코드 삽입 : insert into 테이블명 values( 값1 , 값2 ) , ( 값1 , 값2 );
    -- 4. 검색된 결과를 삽입 : insert into 테이블명 select
		-- 검색된 필드와 삽입할 테이블내 필드명과 동일할 경우
	-- 5. 마지막으로 추가된 레코드의 pk확인 : select last_insert_id();
...
-- update
    ...
    select * from member;
    update member set maddr = 'seoul' where maddr = '서울';	-- 오류 [ char(2) ] / 
    -- 워크벤치에서 update,delete 기본적으로 사용 불가능
		-- 메뉴 -> edit -> preferences -> 사이드메뉴 SQL editor 선택 -> 가장 아래 sage updates 체크해제
...
-- delete
    ...
    -- * 만일 관계테이블 일 경우 다른 테이블에서 fk 필드로 사용된 레코드는 삭제 불가능[ *제약조건 ]
    -- 1. 만약에 핑크가 포함된 이름 삭제
    delete from member where mname like '%핑크%';	-- ??? 오류 [ pk가 포함된 레코드 삭제하면 fk ???? ]
    delete from member where mname = '잇지';			-- ??? 성공 [ 관계가 있는 테이블에서 fk필드 없을경우 ]
/*
	연산자
        ...
        4. 기타연산자 : 
                ...
                값 between 값1 and 값2	:	깂이 값1 부터 값2 사이이면 true [ 값>=값1 and 값<=값2 ]
                like 					:	패턴 검색
					% : 모든 문자수 대응			_ : _개수만큼 문자수 대응
		5. null관련 연산자
            필드명 is null [O]	: null이면	 [ = null  [X] ]
    ...
    집계함수
        ...
        count( 필드명 ) : 해당 필드 내 데이터 수 [ null 미포함 ]
        count(*) : 레코드 수 [ null 포함 ]
*/

-- 타입 확인
drop table if exists testtyep;
create table testtype(
	tinyint_col tinyint ,		-- 1바이트 +-128 저장
    smallint_col smallint , 	-- 2바이트 +-3만정도 저장
    int_col int ,				-- 4바이트 +-21억정도 저장
    int_col2 int unsigned,		-- 4바이트 + 40억정도 저장 [ unsigned : 음수사용X : 음수용량 -> 양수용량증가]
    bigint_col bigint,			-- 8바이트 +-21억이상 저장
    char_col char( 10 ) ,		-- char( 길이 )		: 고정길이 [ 1~255 ] 저장
    varchar_col varchar(10),	-- varchar( 길이 )	: 가변길이 [1~16383] 저장 * mysql 5.0 이상
    text_col text, 				-- 1~65535 정도 글자 저장
