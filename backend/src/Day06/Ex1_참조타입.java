...
public class Ex1_참조타입 {	// c s
	...
	public static void main(String[] args) { // m s
		...
		arr3 = arr2;				// arr2가 가지고 있던 메모리 주소를 arr3에 대입
		
		System.out.println(arr1 == arr2 );	// == 스택영역의 데이터 비교 // 안에 있는 값이 같더라도 주소가 다름 즉 ★주소 비교★
		...
		String str = null;
		//System.out.println(str.length());	// 오류 발생 : 힙영역이 없기때문에 길이체크 불가능
		
		// 3. p. 153
		String hobby = "여행";	// 스택영역에 hobby 이름으로 변수 선언하고 "여행" 이라는 힙의 메모리 주소 저장
		hobby = null;			// hobby 변수의 저장되는값 null 변경하면 힙에 있던 "여행" 메모리는 제거
		
		String kind1 = "자동차";	// '자동차' 힙의 메모리 32번지를 스택 영역의 kind1 저장
