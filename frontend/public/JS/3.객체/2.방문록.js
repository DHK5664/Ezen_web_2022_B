...
function 작성(){ // onclick = "작성()"
	...
	let 내용 = document.querySelector('.내용').value;
		console.log('3.내용 : ' + 내용) 
	let 비밀번호 = document.querySelector('.비밀번호').value;	//1번
	
	// 2. 입력받은 여러개의 데이터를 하나로 객체화
	let board = {writer : 작성자 
				,content : 내용 
				,password : 비밀번호							//2번 
				}
		console.log( board )
...
function 수정( i ){
	// 1. 비밀번호 검증
	let 비밀번호 = prompt('수정할 게시물 비밀번호 입력 : ')
	// 2. 입력받은 비밀번호와 게시물의 비밀번호가 동일하면
	if(비밀번호 == boardArray[i].password){
		// 3. 수정할 내용 입력받아
		let 수정본 = prompt('수정할 내용 입력 : ')
		// 4. 선택한 게시물의 속성의 값 수정
		boardArray[i].content = 수정본;
		alert('수정 성공');
		//5. 화면 새로고침/업데이트
		출력();
	}else{// 같지않으면
...
// boardArray				: 얘는 배열 이지만
// boardArray[i]			: 배열내 i번째 요소 --> 객체 (위에서 배열에 객체를 push했으므로)
