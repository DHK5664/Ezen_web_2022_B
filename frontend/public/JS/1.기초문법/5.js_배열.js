/*	
	데이터[자료] : 문자,숫자,그림 형태의 단위
		자료 증가 --> 메모리 증가 --> 비용 증가 --> 요구사항
	메모리[휘발성] : 사용중에만 저장 / 종료시 사라짐 : JS
		- 보조기억장치[비휘발성] : 데이터베이스
	
	보조기억장치 : SSD/HDD 1. 영구저장[C] 2. 비휘발성 [전기 X , 저장 O]
	변수와 상수	: 데이터[자료] 1개를 메모리[주기억장치]에 저장
	배열 : 데이터 여러개를 저장
		1. 배열 : [  ]
		2. 식별번호 : 인덱스 0번시작
		3. 선언방법
		...
		5. 요소[자료]추가
			배열명.push( 새로운요소 )	: 마지막 요소 뒤에 새로운 요소 추가
			배열명.splice(추가할 인덱스 , 0 , '새로운 요소')	: 해당 인덱스에 새로운 요소 추가 [기존에 있던 요소들은 한 칸씩 뒤로 이동]
			[추가된 인덱스 뒤로 한 칸씩 이동]
		6. 요소 제거
			- 인덱스 이용한 제거
			배열명.splice( 제거할 인덱스의 시작번호, 제거 갯수 , [새로운요소] )[]는 필수가 아니라서 대괄호를 넣었음
...
document.write('<ol>')
document.write('<li>'+배열명[0]+'</li>')	// 변수/배열 문자열 처리X // HTML 문자열 처리 O
document.write('<li>'+배열명[1]+'</li>')	// 변수/배열 문자열 처리X // HTML 문자열 처리 O
document.write('<li>'+배열명[2]+'</li>')	// 변수/배열 문자열 처리X // HTML 문자열 처리 O
document.write('<li>'+배열명[3]+'</li>')	// 변수/배열 문자열 처리X // HTML 문자열 처리 O
document.write('</ol>')
// 4. 배열의 길이
console.log(배열명.length +"명")
document.write('<p> 현재인원수 : ' + 배열명.length+'명</p>')
// 5. 배열의 요소 추가
배열명.push('전현무')
console.log(배열명)
// 6. 배열의 특정 요소 제거	
...
// 9. 배열 내 요소 데이터 변경
배열명.splice(0 , 1 , '서장훈'); console.log(배열명)		//['서장훈','김현수','전현무']

// 10. 베열 내 요소 사이 [서장훈과 김현수 사이] 요소 추가
배열명.splice(1 , 0 , '유재석')
