...
console.log(output)
//풀이 :line8에 임의로 숫자 5를 대입하면  i=줄 개수 이므로 5줄이 생기고 역 피라미드 모양이다보니 위에서 내려갈수록
 //좁아지는 모양이 나와야 함 그래서 s<=i일때와 반대로 s<=line8 대입하면 ***** 이거로만 나오게 됨
 //그런데 양쪽에서 좁아지면서 내려가야 해서 공백값을 s위에 먼저 넣어주고 5줄일때 첫줄은 별의 개수가 9 두번째는 7 이런식으로 나와야 하므로
 //기입한 줄 개수(line8)*2= 각 줄마다 10이 될거고 - i를 넣으면 첫줄은 10-1=9 두번째는 10-2=8 이런식으로 된다.
 //하지만 9 7 5 3 1 일케 나와야하니까 line8*2-i*2= 8 6 4 2 0 이 되니까 뒤에 +1만 더 해주면 9 7 5 3 1 일케 대서 원하던 대로 역피라미드를 만들 수 있게 된다.
*/
