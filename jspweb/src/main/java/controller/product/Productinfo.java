...
public class Productinfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		파일저장소.setSizeThreshold(1024*1024*10);	// 파일저장소에 저장할 수 있는 최대 용량 범위
		파일저장소.setDefaultCharset("UTF-8");		// 파일저장소 한글타입 인코딩
			...
			for(FileItem item :파일아이템목록) {	// 요청된 모든 매개변수들을 반복문 돌려서 확인
				...
				if(item.isFormField()) {
					...
					// 9. 첨부파일 이름 식별이름 변경	
						...
						// 최종 식별 파일명 : UUID 파일명
					String filename = UUID.randomUUID() +" "+ (item.getName().replaceAll(" ", "-"));	// 파일명에 띄어쓰기 있으면 자르기가 안대서 없앰
