...
public class Board extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 3. Dto 객체	[ 기본값 : int 필드 0 / 객체 필드 null (String은 객체이므로 null) ]
		...
		// 5. Dao 결과인 true , false 데이터를 response 객체 이용한 응답
			// 'true' vs '{true}' ↓
			// true 얘처럼 결과 1개이면 걍 얘처럼 보내는게 맞음 vs { 1: true , 2: false , 3: true 이런식으로 결과가 여러개면 객체화가 맞음 }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String jsonArray = mapper.writeValueAsString(result);	// 2. DAO로부터 받은 list를 json형식의 문자열로 변환하기
...
/*
  	...
  	★ JSP(얘 에서만 그럼) 서블릿 HTTP 메소드 put , delete 사용시 필수 설정! ★
  		* 기본값 : 매개변수 요청 차단
  	1. Servers 폴더 클릭
  	2. server.xml 더블클릭
  	3. 하단 Source 탭 클릭
