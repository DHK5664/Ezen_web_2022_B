...
public class Ex2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Dto> list = Dao.getInstance().getData2();
			...
			// * JAVA객체 와 JS객체는 체계/형태가 다르다 [ 호환 불가 ]
			// 해결 : JAVA객체를 JS객체로 바꾸자 [ Object -> json ]
			// 1. 라이브러리 3개 필요 [jackson-databind-x , jackson-core-x , jackson-annotations-x ]
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2.request.getParameter("매개변수명");	: 데이터 가져올때 무조건 String 타입 가져온다. [ 필요에 따라 형변환 필요 ]
